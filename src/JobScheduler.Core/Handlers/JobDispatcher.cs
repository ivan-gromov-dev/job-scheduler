...
internal sealed class JobDispatcher(IServiceProvider services, JobHandlerRegistry registry) : IJobDispatcher
{
    public Task DispatchAsync(Job job, CancellationToken cancellationToken)
    {
        ...
        return registry.TryGet(job.Type, out var registration)
            ? registration.Dispatch(this, job.Payload, cancellationToken)
    ...
    {
        var message = JsonSerializer.Deserialize<TJob>(payload)
            ?? throw new JsonException($"Payload for '{JobTypeName.For<TJob>()}' deserialized to null.");
